...
secret_threshold=$(curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --silent --request GET --header "X-Consul-Token:$CONSUL_ACL_TOKEN" $CONSUL_URI/v1/kv/vault_keys/secret_threshold?raw)
if [[ -z secret_shares ]]
then
...
do
    key=$(curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --silent --request GET --header "X-Consul-Token:$CONSUL_ACL_TOKEN" $CONSUL_URI/v1/kv/vault_keys/public_keys/pgp_keys/key$i.key?raw | tr -d '\n')
    if [[ -z secret_shares ]]
    then
...
pgp_keys="$pgp_keys]"
root_token_pgp_key=$(curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --silent --request GET --header "X-Consul-Token:$CONSUL_ACL_TOKEN" $CONSUL_URI/v1/kv/vault_keys/public_keys/root_token_pgp_key/key0.key?raw | tr -d '\n')
if [[ -z secret_shares ]]
then
...
initResponse=$(curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --request PUT --data "$initData" http://vault:8200/v1/sys/init)
...
do
  curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --silent --request PUT --header "X-Consul-Token: $CONSUL_ACL_TOKEN" --data "$key" $CONSUL_URI/v1/kv/vault_keys/seal_keys/key$index.base64.txt
  let index=$index+1
done

curl --key /consul/certs/privkey.pem --cert /consul/certs/cert.pem --silent --request PUT --header "X-Consul-Token: $CONSUL_ACL_TOKEN" --data "$root_token" $CONSUL_URI/v1/kv/vault_keys/root_token.cipher.txt
